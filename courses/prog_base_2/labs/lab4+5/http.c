...
http_request_parse(const char * const request) {
    ...
    char uri[256] = "";
    memcpy(uri, uriStartPtr, uriLen);
    const char * uriEndPtrHelp = strstr(uri, "?");
    if(uriEndPtrHelp != NULL) {
        ptrdiff_t uriLen = uriEndPtrHelp - uri;
        memcpy(req.uri, uri, uriLen);
        req.uri[uriLen] = '\0';
        const char * cur = uriEndPtrHelp;
        cur++;
        while (strlen(cur) > 0) {
            const char * pairEndPtr = strchr(cur, '&');
            if (NULL == pairEndPtr) {
                pairEndPtr = cur + strlen(cur);
            }
            keyvalue_t kv;
            // get key
            char * eqPtr = strchr(cur, '=');
